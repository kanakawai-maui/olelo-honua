...
export class MultiLanguageProvider implements LanguageProvider {
    ...
    constructor(providers: LanguageProvider[], agreementThreshold: number = 0.95) {
        this.providers = providers;
        this.agreementThreshold = agreementThreshold;
    }

    async translateText(text: string, from: string, to: string): Promise<string> {
        const translations = await Promise.all(this.providers.map(provider => provider.translateText(text, from, to)));
        ...
        if (agreementCount / totalTranslations < this.agreementThreshold) {
            throw new Error('Translations do not agree by at least the configured threshold');
        }
