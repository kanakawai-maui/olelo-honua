...
const mockProvider: LanguageProvider = {
    translateText: jest.fn((text, from, to) => Promise.resolve(`Translated ${text} from ${from} to ${to}`))
};
...
describe('OleloHonua', () => {
    ...
    test('should throw error if prime language is not specified', () => {
        ...
        expect(() => oleloHonua['validateConfig'](invalidConfig)).toThrow("Prime language must be specified.");
    });
    ...
    test('should throw error if both includeLanguage and excludeLanguage are specified', () => {
        ...
        expect(() => oleloHonua['validateConfig'](invalidConfig)).toThrow("Only one of includeLanguage or excludeLanguage must be specified, not both.");
    });

    test('should throw error if neither includeLanguage nor excludeLanguage are specified', () => {
        const invalidConfig = { ...mockConfig, includeLanguage: [], excludeLanguage: [] };
        expect(() => oleloHonua['validateConfig'](invalidConfig)).toThrow("One of includeLanguage or excludeLanguage must be specified.");
    });
});